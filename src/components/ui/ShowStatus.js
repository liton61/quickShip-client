...
const ShowStatus = ({ trackingId }) => {
    ...
    const getPayment = payment.find(payment => payment._id === trackingId);
    ...
    const getStatusStep = () => {
        ...
        if (OrderId?.status === "delivered") {
            ...
            return 3; // On the way
        } else if (getPayment?.payment === "successfully") {
            return 2; // Payment
        ...
        <div>
            {OrderId ? (
                <ul className="steps steps-vertical lg:steps-horizontal w-full h-full">
                    <li data-content={getStatusStep() === 1 ? "✓" : ""} className={`step ${getStatusStep() >= 1 ? 'step-info' : ''}`}>
                ...
                <p className='text-red-600 font-semibold text-lg'>Please, provide valid tracking Id !</p>
            )}
        </div>
    );
};
